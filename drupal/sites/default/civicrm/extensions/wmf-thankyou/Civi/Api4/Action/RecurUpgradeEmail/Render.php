...
/**
 * Class Render. This is mostly copy-pasted from the FailureEmail in the Smashpig extension
 *
...
class Render extends AbstractAction {
  ...
  public function _run(Result $result) {
    $email = Email::get()
      ...
      ->addWhere('email', '<>', '')
      ->setSelect(['contact_id.preferred_language', 'email', 'contact_id.display_name', 'contact_id.first_name'])
  ...
  protected function getSupportedLanguages(): array {
    if (!isset(Civi::$statics[__CLASS__]['languages'])) {
      $templateID = Civi\Api4\MessageTemplate::get(FALSE)->addWhere('workflow_name', '=', 'recurring_upgrade_message')->addSelect('id')->execute()->first()['id'];
