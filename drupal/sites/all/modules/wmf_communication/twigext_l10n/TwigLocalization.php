...
class TwigLocalization extends Twig_Extension {
  ...
  public function getFilters() {
    return [
      new Twig_SimpleFilter('l10n_currency', [$this, 'l10n_currency']),
      new Twig_SimpleFilter('l10n_number', [$this, 'l10n_number']),
    ];
  }

  /**
   * Localize a currency string, string is expected to be <ISO Code> <Non
   * formatted number> Will transform the string by replacing the currency
   * symbol, if available, formatting the number and re arranging the
   * symbol/number as appropriate to the language.
   *
   * @param string $string <ISO Code> <Non formatted number [0-9]+[\.]?[0-9]*>
   * @param string $locale Language to translate into, may be a locale (e.g.
   *   en_GB)
   *
