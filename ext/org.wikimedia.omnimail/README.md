...
| **OmniContact.get** | v4 | Retrieve real-time contact snapshot from Acoustic/Silverpop.                                                | `email`, `contact_id`, `group_identifier`, `recipientID`       | e.g wmf-cv api4 Omnicontact.get recipientID=748572098415                                                         |
| **OmniContact.create** | v4 | Push contact updates to provider (currently supports **group fields**, **snooze**, **is_orphan**).          | `email` OR  `recipientID`, `values`, `group_id`                              | Update or create acoustic record. e.g     wmf-cv api4 Omnicontact.create recipientID=748572098447 +v is_orphan=1 |
