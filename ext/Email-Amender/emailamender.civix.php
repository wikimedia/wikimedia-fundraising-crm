 ...
 * extension.
 */
class CRM_Emailamender_ExtensionUtil {
  const SHORT_NAME = 'emailamender';
  const LONG_NAME = 'uk.org.futurefirst.networks.emailamender';
  const CLASS_PREFIX = 'CRM_Emailamender';
 ...
 * @return bool
 */
function _emailamender_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _emailamender_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _emailamender_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _emailamender_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _emailamender_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _emailamender_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _emailamender_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _emailamender_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
