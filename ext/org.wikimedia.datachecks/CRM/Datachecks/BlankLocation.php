...
class CRM_Datachecks_BlankLocation extends  CRM_Datachecks_LocationBase {
  ...
  public function check() {
    ...
    foreach ($this->entities as $entity) {
      $count = CRM_Core_DAO::singleValueQuery("SELECT count(*) FROM civicrm_{$entity}
                WHERE location_type_id IS NULL OR location_type_id = 0");
      if ($count) {
        $examples = [];
        $exampleContactsDAO = CRM_Core_DAO::executeQuery(
          "SELECT contact_id FROM civicrm_{$entity}
                  WHERE location_type_id IS NULL OR location_type_id = 0 ORDER BY id DESC LIMIT 5");
    ...
    foreach ($this->entities as $entity) {
      foreach ($this->getLocationTypes() as $locationTypeID) {
        ...
        $temporaryTable = CRM_Utils_SQL_TempTable::build()->createWithQuery("
           ...
           WHERE (t1.location_type_id IS NULL OR t1.location_type_id=0) AND t2.id IS NULL;
        ")->getName();
