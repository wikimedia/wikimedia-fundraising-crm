...
class CRM_Assignee_ExtensionUtil {
  ...
  const LONG_NAME = "uk.squiffle.assignee";
 ...
 * @return array(string)
 */
function _assignee_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_assignee_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
...
function _assignee_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_assignee_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _assignee_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _assignee_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 */
function _assignee_civix_glob($pattern) {
  $result = glob($pattern);
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _assignee_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _assignee_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _assignee_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _assignee_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
...
function _assignee_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _assignee_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
