 ...
 *
 * Then IF there is a fee against the record we ALSO need some extra rows.
 * However, per comment at https://gerrit.wikimedia.org/r/#/c/266468/3/sites/all/modules/wmf_civicrm/update_7070.php
 * and ticket https://issues.civicrm.org/jira/browse/CRM-17890 I decided not to add these.
 *
 * Although it would appear the upgrade script *should* have added them, newly refunded contributions were
 * not getting them, so adding them makes them less consistent with others & harder to deal with later.
 * However, I am recording what is required here:
...
function _wmf_civicrm_update_7070_fill_refund_transaction_data() {
  ...
  while ($result->fetch()) {
    ...
    try {
      ...
      $params = array(
        1 => array($result->currency, 'String'),
        2 => array($result->trxn_id, 'String'),
        3 => array($result->check_number, 'String'),
      );
