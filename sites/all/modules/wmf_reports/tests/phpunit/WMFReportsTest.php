...
class WMFReportsTest extends BaseWmfDrupalPhpUnitTestCase {
     ...
     *
     * This test doesn't check what is retrieved - just that the reports can be run
     * without error.
     *
    ...
    public static function getReportTemplates() {
        return array(array('contribute/wmf_lybunt'), array('contribute/reconciliation'), array('contribute/trends'), array('contribute/recur'), array('contribute/detail'));
    }

    /**
     * Ported function from core that prevents enotices & allows tests to complete.
     *
     * NB - it's really annoying not to use the core function directly but we would need to do restructuring in core,
     * which, while desirable is not short term.
     *
