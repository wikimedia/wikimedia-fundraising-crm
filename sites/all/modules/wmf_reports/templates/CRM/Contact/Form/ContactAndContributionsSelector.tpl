{*
  Based on civicrm/templates/CRM/Contribute/Form/Selector.tpl
  Changed to display a contact row followed by contribution rows.
*}
...
<table class="selector">
  ...
  <tr id="rowid{$row.contribution_id}" class="{cycle values="odd-row,even-row"}{if $row.cancel_date} disabled{/if} crm-contribution_{$row.contribution_id}">
    ...
    {if $row.contact_type}
        ...
        <td colspan=3>
        <ul>
        {foreach from=$row.all_addresses item=address}
            <li>{$address}
        {/foreach}
        </ul>
        </td>
        <td colspan=2>
        <ul>
        {foreach from=$row.all_phones item=phone}
            <li>{$phone}
        {/foreach}
        </ul>
        </td>
